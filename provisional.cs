    ...
    public partial class provisional : Form
    {
      ...
      //              ManagerBox.Text = row["Note"].ToString();
            ...
            foreach (DataRow row in Current_provisional.Rows)  // չգրանցված պատվերները գռանցվում են  provisional - ում
            {
                ...
                string InsertQuery = $"INSERT provisional SET `Code` = '{row["code"]}',`Name`='{row["name"]}',`Seans`='{seans_state}',`Ticket`='{tick}'," +
            ...
            DataRow[] foundRowsTI = Ticket_Provisional.Select($"`Nest`= '{nest.Text}' AND `Ticket`= '{tick}' AND `Seans` = '{seans_state}' ");
